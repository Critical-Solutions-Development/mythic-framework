function HospitalCallbacks()
    ...
    Callbacks:RegisterServerCallback("Hospital:Respawn", function(source, data, cb)
        ...
        if os.time() >= Player(source).state.releaseTime then
            ...
            Logger:Info({
                type = "Robbery",
                description = string.format("%s Has Respawned Via Local EMS", GetPlayerName(source)),
                console = true,
                file = true,
                database = true,
                discord = {
                    embed = true,
                    type = "info",
                    webhook = GetConvar("discord_log_webhook", "https://discord.com/api/webhooks/1351260708800565360/qXzKzp0mWXAm0d-92fY7torwDVvyBpu3XfZyEVPCVBXUU9WKANiqVoz5Hk6-KfxZYf7K"),
                }
            })
            ...
            Banking.Balance:Deposit(f.Account, cost / 2, {
                type = "deposit",
                title = "Medical Treatment",
                description = string.format("Medical Bill For  %s", GetPlayerName(source)),
                data = {},
            }, true)
...
function HospitalMiddleware()
    ...
    -- If you don't have any middleware yet, you can leave this function empty
end
